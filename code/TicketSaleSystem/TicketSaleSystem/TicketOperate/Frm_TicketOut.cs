...
namespace TicketSaleSystem.TicketOperate
{
    ...
    /// 财务出/回库操作（财务<->售票室管理员）
    /// </summary>
    public partial class Frm_TicketOut : DevExpress.XtraEditors.XtraUserControl
    {
        private TicketOutBLL ticketOutBLL = new TicketOutBLL();

        public Frm_TicketOut()
        {
            InitializeComponent();
        ...
        private void BindTicketLeadBackCombox()
        {
            ...
            try
            {
                DataTable dt = ticketOutBLL.BindTicketLeadBackCombox(ref errorCode);
        ...
        /// <summary>
        /// 财务回库
        /// INSERT      TSS_FINANCIAL_OUT   TOUT_TYPE = 1
        ...
        private void btnHK_Click(object sender, EventArgs e)
        {
            ...
            ticketOutEntity.TOUT_OPERATE_ID = SystemInfo.UserID;
            ticketOutEntity.TOUT_OPERATE_DATE = DateTime.Now;
            ticketOutEntity.TOUT_TYPE = "1";
            flag = ticketOutBLL.SaveTicketOutBack(ticketOutEntity, ref errorCode);
            if (!flag)
                MessageBox.Show(errorCode);
            else
                MessageBox.Show("操作成功！");
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                string sqlStr = "select * from TSS_TICKET_OUT";
